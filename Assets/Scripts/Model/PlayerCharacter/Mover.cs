...
namespace Dungeons.Model.PlayerCharacter
{
    public class Mover : MonoBehaviour
    {
        ...
        {
            _rigidbody2d.velocity = Vector2.Lerp(_rigidbody2d.velocity, GetDirectionVector(_player.States.RightDirection) * GetCurrentDeltaSpeed(_player.Data, _player.States.PowerState),
        ...
        public void JumpUp()
        {
            _rigidbody2d.AddForce(Vector2.up * GetCurrentDeltaJumpForce(_player.Data, _player.States.PowerState), ForceMode2D.Impulse);
        }

        public void JumpForward()
        {
            _rigidbody2d.velocity = Vector2.zero;
            _rigidbody2d.AddForce(GetCurrentDeltaTotalForce(_player.Data, _player.States.RightDirection, _player.States.PowerState), ForceMode2D.Impulse);
        }
        ...
        private float GetCurrentDeltaSpeed(PlayerData playerBase, PowerState powerState)
        {
            return playerBase.GetSpeedForPowerState(powerState) * Time.deltaTime;
        }

        private float GetCurrentDeltaJumpForce(PlayerData playerBase, PowerState powerState)
        {
            return playerBase.GetJumpForceForPowerState(powerState) * Time.deltaTime;
        }

        private Vector2 GetCurrentDeltaTotalForce(PlayerData playerBase, bool rightDirection, PowerState powerState)
        {
