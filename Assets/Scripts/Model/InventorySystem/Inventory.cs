...
namespace Dungeons.Model.InventorySystem
{
    public class Inventory : MonoBehaviour
    {
        ...
        [SerializeField] private InventoryNotifier _notifier;
        ...
        public bool TryToTakeInHands(Item item)
        {
            if (_handsSlot.TrySet(item) == false)
            {
                ...
                {
                    _notifier.FullInventoryShow();
                    return false;
                }
            }

            _notifier.TakingItemInHandsShow(item);
            return true;
        }
        ...
        {
            _notifier.ItemsTakenNumberShow(Mathf.Min(itemsNumber, emptySlotsNumber));

            int notPickedUpItems = itemsNumber - emptySlotsNumber;
            if (notPickedUpItems > 0)
                _notifier.NotFitIntoInventoryItemsNumberShow(notPickedUpItems);
        }
